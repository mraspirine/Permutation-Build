...
- **Placement:** flow band at `y≈147–4200` · permutation band at **`y≈6176`** · boards ordered by x following the screen order in the flow (leaving gaps for screens that have no board yet) · the band belongs to the SECTION that holds the flow's boards — a base sitting in a sibling section with no boards (Payment 29.1) is the *shared band in another section* case → ask (Phase 4 step 3)
...
> A Tier-3 case that recurs gets its row here the first time it is minted; the next board copies it. "On boards" names the FLOW too — another flow's screen with the same number (Payment 29.1) has no board until it is built. The ids themselves are flow-agnostic: the same situation on another flow's screen reuses the id verbatim.
