...
- **G1 — วัดก่อนสร้าง** ทุกโปรเจควาง board ไม่เหมือนกัน (ระยะ ฟอนต์ line-height
  รูปแบบ caption เส้นโยง) และโปรเจคเดียวกันยังต่างกันราย flow ได้ skill เลยวัด **board ข้างเคียงใน section เดียวกัน (ไม่มีก็ใน flow เดียวกัน)**
...
- **G5 — ตรวจก่อนจบ** `verify-board.js` เช็คให้หมด: จำนวนเคส เลขซ้ำ รูปแบบ label
  ...
  (นับ node ก่อน-หลังเทียบกัน) ยังไม่ `pass: true` หรือ `stats.skipped` ยังไม่ว่าง = ยังไม่เสร็จ
